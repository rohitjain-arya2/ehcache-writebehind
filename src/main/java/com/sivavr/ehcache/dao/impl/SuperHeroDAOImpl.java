...
public class SuperHeroDAOImpl implements SuperHeroDAO {
	...
	public SuperHero Save(SuperHero superHero) {
		LOGGER.info("*** Accessing Dao Layer: SuperHeroDAOImpl.Save() ***");		
		session = sessionFactory.openSession();
		tx = session.beginTransaction();
		LOGGER.info("*** before Accessing Dao Layer: SuperHeroDAOImpl.Save() ***"+superHero.getId());		

		if(superHero.getId()== 0 || superHero.getId()== 1 )
			superHero.setId(new Long(findAll().size()+1));
		LOGGER.info("*** After Accessing Dao Layer: SuperHeroDAOImpl.Save() ***"+superHero.getId());		
	...
	public Long getsIncrement() {		
		session = sessionFactory.openSession();
		tx = session.beginTransaction();
		Query q = session.createQuery("select count(*) from super_heros");
		List result = q.list();
